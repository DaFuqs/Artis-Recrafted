...
public record ArtisScreenFactory(ArtisTableType tableType, Block block,
	...
	public @NotNull ScreenHandler createMenu(int syncId, PlayerInventory inv, PlayerEntity player) {
		if (this.block == null || this.blockHitResult == null) {
			return new ArtisRecipeProvider(Registries.SCREEN_HANDLER.get(tableType.getId()), tableType, syncId, player, ScreenHandlerContext.create(player.world, player.getBlockPos()));
		} else {
			return new ArtisRecipeProvider(Registries.SCREEN_HANDLER.get(tableType.getId()), tableType, syncId, player, ScreenHandlerContext.create(player.world, blockHitResult.getBlockPos()));
		}
	}
