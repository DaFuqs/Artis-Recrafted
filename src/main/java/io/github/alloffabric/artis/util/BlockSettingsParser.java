...
public class BlockSettingsParser {
	...
	public static Block.@NotNull Settings parseSettings(JsonObject json) {
		...
		if (json.containsKey("copy")) {
			...
			settings = FabricBlockSettings.copyOf(Registries.BLOCK.get(new Identifier(copyBlockArg)));
		...
		if (json.containsKey("drops_nothing")) {
			...
			settings.dropsLike(Registries.BLOCK.get(new Identifier(json.get(String.class, "drops_like"))));
